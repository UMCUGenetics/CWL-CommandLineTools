...
inputs:
  ...
  - id: crop
    ...
    doc: >
      ...
      the specified length after this step has been performed. Steps performed
      after CROP might of
  ...
  - id: log_filename
    ...
    doc: >
      ...
      Specifying a trimlog file creates a log of all read trimmings, indicating
  ...
  - id: maxinfo
    ...
    doc: >
      ...
      balance between preserving as much read length as possible vs. removal of
      incorrect

      bases. A low value of this parameter (<0.2) favours longer reads, while a
      high value
  ...
  - id: minlen
    ...
    doc: >
      ...
      This module removes reads that fall below the specified minimal length. If
      required, it should

      normally be after all other processing steps. Reads removed by this step
      will be counted and

      included in the "dropped reads" count presented in the trimmomatic
      summary.
  ...
  - default: '64'
    ...
    doc: >
      ...
      -phred33 ("33") or -phred64 ("64") specifies the base quality encoding.
      Default: -phred64
  - id: slidingwindow
    type: string?
    inputBinding:
  ...
  - id: trailing
    ...
    doc: >
      ...
      Remove low quality bases from the end. As long as a base has a value below
      this threshold

      the base is removed and the next base (which as trimmomatic is starting
      from the 3" prime end

      would be base preceding the just removed base) will be investigated. This
      approach can be

      used removing the special illumina "low quality segment" regions (which
      are marked with
...
doc: >
  ...
  Illumina (FASTQ) data as well as to remove adapters. These adapters can pose a
  real problem
  ...
  paired end mode will maintain correspondence of read pairs and also use the
  additional

  information contained in paired reads to better find adapter or PCR primer
  fragments

  introduced by the library preparation process.

  Trimmomatic works with FASTQ files (using phred + 33 or phred + 64 quality
  scores,
